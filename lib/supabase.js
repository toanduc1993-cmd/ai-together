...
// ============ EPICS ============
export async function getEpicsByProject(projectId) {
    const { data, error } = await supabase.from('epics').select(`
        *,
        creator:users!epics_created_by_fkey(id, display_name, avatar, username)
...
export async function getModulesByProject(projectId) {
    // Try with module_assignees join; fallback if table doesn't exist yet
    let data, error;
    ({ data, error } = await supabase.from('modules').select(`
    *,
    assignee:users!modules_assigned_to_fkey(id, display_name, avatar, username),
    module_assignees(id, user_id, users:users!module_assignees_user_id_fkey(id, display_name, avatar, username))
  `).eq('project_id', projectId).order('created_at'));
    if (error) {
        // Fallback without module_assignees join
        ({ data, error } = await supabase.from('modules').select(`
      *,
      assignee:users!modules_assigned_to_fkey(id, display_name, avatar, username)
    `).eq('project_id', projectId).order('created_at'));
        if (error) throw error;
        return (data || []).map(m => ({ ...m, assignees: m.assignee ? [m.assignee] : [] }));
    }
    return (data || []).map(m => ({
        ...m,
        assignees: (m.module_assignees || []).map(a => a.users).filter(Boolean),
    }));
}

// Get ALL modules across all projects (single query, avoids N+1)
...
// ============ MODULE ASSIGNEES ============
export async function getModuleAssignees(moduleId) {
    const { data, error } = await supabase.from('module_assignees').select(`
    *,
    user:users!module_assignees_user_id_fkey(id, display_name, avatar, username)
  `).eq('module_id', moduleId);
    if (error) return []; // table may not exist yet
    return data;
}

export async function setModuleAssignees(moduleId, userIds) {
    // Delete existing
    await supabase.from('module_assignees').delete().eq('module_id', moduleId);
    // Insert new
    if (userIds && userIds.length > 0) {
        const rows = userIds.map(uid => ({ module_id: moduleId, user_id: uid }));
        const { error } = await supabase.from('module_assignees').insert(rows);
        if (error) throw error;
    }
    // Keep assigned_to in sync (first assignee)
    await supabase.from('modules').update({ assigned_to: userIds?.[0] || null }).eq('id', moduleId);
}
...
export async function deleteModule(id) {
    ...
    await supabase.from('checklist_items').delete().eq('module_id', id);
    await supabase.from('deliverables').delete().eq('module_id', id);
    // Delete module
